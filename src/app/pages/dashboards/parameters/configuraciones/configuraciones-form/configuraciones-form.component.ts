...
export class ConfiguracionesFormComponent implements OnInit {
	...
	breadcrumb = [
		...
		{ name: 'Configuración General', icon: 'fa-duotone fa-gear' },
	];
	public listaTamano: any[] = [{ valor: 80 }];

	constructor(public routerActive: ActivatedRoute, private service: GeneralParameterService, private helperService: HelperService) {
		this.frmConfiguracion = new FormGroup({
			...
			ExtensionLogo1: new FormControl(''),
			ExtensionLogo2: new FormControl(''),
			ExtensionCertificacion: new FormControl(''),
			ExtensionRepresentante: new FormControl(''),
			ExtensionBackground: new FormControl(''),
		});
		...
		this.service.save('Configuracion', this.id, data).subscribe(
			(response) => {
				if (response.status) {
					...
					this.helperService.showMessage(MessageType.SUCCESS, 'Ajustes guardados');
				} else {
					setTimeout(() => {
						this.helperService.hideLoading();
					}, 200);
	...
	fileEvent(event: any) {
		...
		if (type == 'png' || type == 'jpeg' || type == 'jpg') {
			...
			reader.onload = async (e: any) => {
				...
				this.frmConfiguracion.controls['ExtensionBackground'].setValue(type);
				this.img = archivo;
			};
		}
	}
	...
	fileEventRepresentante(event: any) {
		...
		if (type == 'png' || type == 'jpeg' || type == 'jpg') {
			...
			reader.onload = async (e: any) => {
				...
				this.frmConfiguracion.controls['ExtensionRepresentante'].setValue(type);
	...
	fileEventLogo1(event: any) {
		...
		if (type == 'png' || type == 'jpeg' || type == 'jpg') {
			...
			reader.onload = async (e: any) => {
				...
				this.frmConfiguracion.controls['ExtensionLogo1'].setValue(type);
	...
	fileEventLogo2(event: any) {
		...
		if (type == 'png' || type == 'jpeg' || type == 'jpg') {
			...
			reader.onload = async (e: any) => {
				...
				this.frmConfiguracion.controls['ExtensionLogo2'].setValue(type);
	...
	fileEventCertificacion(event: any) {
		...
		if (type == 'png' || type == 'jpeg' || type == 'jpg') {
			...
			reader.onload = async (e: any) => {
				...
				this.frmConfiguracion.controls['ExtensionCertificacion'].setValue(type);
...
@NgModule({
	...
	imports: [CommonModule, GeneralModule, NgbAccordionModule],
})
